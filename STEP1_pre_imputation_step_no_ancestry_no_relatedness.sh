...
awk '{if ($5 == 1) print $2 }' call_rates.lmiss > snp_outliers.txt

mv call_rates.lmiss log/CALL_RATES_ALL_VARIANTS.txt
rm call_rates.lmiss
rm call_rates.log
rm call_rates.hh
rm call_rates.nosex
...
plink --bfile $FILENAME --remove all_outliers.txt --exclude snp_outliers.txt --make-bed --out after_heterozyg

mv HETEROZYGOSITY_OUTLIERS.txt log
...
#plink --bfile after_gender --flip HAPMAP_bin_snplis-merge.missnp --make-bed --out after_gender3

#plink --bfile after_gender --bmerge 1000G_EUR --out 1000G_bin_snplis --make-bed
#plink --bfile after_gender --flip 1000G_bin_snplis-merge.missnp --make-bed --out after_gender3
#plink --bfile after_gender3 --bmerge 1000G_EUR --out 1000G_bin_snplis --make-bed
#plink --bfile after_gender3 --exclude 1000G_bin_snplis-merge.missnp --out after_gender4 --make-bed
#plink --bfile after_gender4 --bmerge 1000G_EUR --out 1000G_bin_snplis --make-bed
#plink --bfile 1000G_bin_snplis --geno 0.05 --out pca --make-bed --pca --threads $THREADS
...
#grep -f <(grep "EUR" 1000G_samples.tsv | cut -d '	' -f1) pca.eigenvec > eur.txt
#grep -f <(grep "ASN" 1000G_samples.tsv | cut -d '	' -f1) pca.eigenvec > asia.txt
#grep -f <(grep "AFR" 1000G_samples.tsv | cut -d '	' -f1) pca.eigenvec > afri.txt
...
plink --bfile after_gender --make-bed --out after_gender_heterozyg_pihat_mind --geno 0.05
...
Rscript get_database_info_short.R $DATABASE_INFO
